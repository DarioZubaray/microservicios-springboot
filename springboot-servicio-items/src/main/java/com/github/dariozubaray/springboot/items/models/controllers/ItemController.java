...
//    @Qualifier("ItemRestTemplate")
    ...
    public ResponseEntity<?> obtenerConfiguracion(@Value("${server.port}") String puerto) {
        ...
        respuestaJson.put("texto", texto);
        respuestaJson.put("puerto", puerto);

        if (env.getActiveProfiles().length > 0 && env.getActiveProfiles()[0].equals("dev") ) {
            respuestaJson.put("autor.nombre", env.getProperty("configuracion.autor.nombre"));
            respuestaJson.put("autor.email", env.getProperty("configuracion.autor.email"));
        }

        return new ResponseEntity<Map<String, String>>(respuestaJson, HttpStatus.OK);
    }
